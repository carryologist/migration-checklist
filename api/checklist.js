...
function getRedis() {
  if (!redis) {
    // Vercel native Redis provides KV_URL (rediss://), KV_REDIS_URL, or similar
    const url = process.env.KV_URL || process.env.KV_REDIS_URL;
    if (!url) throw new Error("No Redis URL. Found env vars: " + Object.keys(process.env).filter(k => k.includes('KV') || k.includes('REDIS')).join(', '));
    redis = new Redis(url, {
      connectTimeout: 5000,
      commandTimeout: 5000,
